...
{
  int idet = threadIdx.z + blockDim.z * blockIdx.z; 
  int p = threadIdx.x + blockDim.x * blockIdx.x;
  int i = threadIdx.y + blockDim.y * blockIdx.y;
  if (idet >= ndet || p >= nelec || i >= nelec)
    return;
  // for (i = 0 : nelec) R[nd][i][i] = w
  if (p < nact & p == i) {
    Rbuff[idet*nact*nelec + p*nelec + i] = weights[idet];
  }
  __syncthreads();
 ...
 // NEED an extra index !~!!!
  if (p < nex && i < nelec) {
    for (int q = 0; q < nex; ++q) {
      Rbuff[idet*nact*nelec + p*nelec + i] -= weights[idet] * I[idet*nex*nex + p*nex + q] * T[q*nmo + i];
    }
  }
  __syncthreads();
  if (p < nex && i < nex) {
    Rbuff[idet*nact*nelec + p*nelec + i] += weights[idet] * I[idet*nex*nex + p*nex + i];
  }
  __syncthreads();
  // Reduce into R[p,q] = \sum_idet R[idet, p, q]
  /*if (idet == 0 && p < nact && i < nelec) {*/
    /*for (int id=0; id < ndet; id++) {*/
      /*R[p*nelec + i] += Rbuff[id*nact*nelec + p*nelec + i]; */
      /*[>if (p == 0 && i == 0)<]*/
        /*[>printf("this : %d %d %d %f\n", p, i, id, Rbuff[id*nact*nelec + p*nelec + i].real());<]*/
    /*}*/
  /*}*/
  /*__syncthreads();*/
...
__global__ void kernel_reduce_phmsd_R(int ndet,
                                      int nex,
                                      int nact,
                                      int nelec,
                                      int nmo,
...
{
  int xblock_dim = 16;
  int grid_dim_x = (nact + xblock_dim - 1) / xblock_dim;
                       ...
                       std::complex<double>* R)
{
  ...
  kernel_reduce_phmsd_R<<<grid_dim, block_dim>>>(ndet, nex, nact, nelec, nmo, iexcit, orbs,
                                                 reinterpret_cast<thrust::complex<double> const*>(weights),
                                                 reinterpret_cast<thrust::complex<double> const*>(T),
                                                 reinterpret_cast<thrust::complex<double> const*>(I),
                                                 reinterpret_cast<thrust::complex<double>*>(Rbuff),
